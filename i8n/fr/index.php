<?php
//Expanse Internationalzation Language File - Français : Global 
//Version: 1.1.0

//Main Page : Heading
$i8n['main_headline']='plateforme blockchain<br />décentralisée et communautaire';
$i8n['main_description']='Expanse est une plateforme cryptographique d\'informations, d\'applications, et de contrats. Le projet est géré par ses utilisateurs au travers d\'une organisation communautaire décentralisée, ouverte et équitable.';
$i8n['main_readmore']='read more';

//Main Page : About
$i8n['main_about_what']='Qu\'est ce que Expanse?';
$i8n['main_about_left']='Expanse est une plateforme cryptographique d\'informations, d\'applications, et de contrats. Le projet Expanse est géré par sa communauté au travers d\'une DAO (Organisation Autonome Décentralisée) au sein de laquelle ses membres collaborent directement, partagent des idées et des concepts, et votent pour déterminer le développement futur du projet et prendre les décisions managériales.';
$i8n['main_about_right']='Mettant en avant un panel grandissant de Contrats Intelligents, les dApps et toolkits fournissent aux utilisateurs et organisations de tout type un accès intuitif à l\'une des applications les plus enthousiasmantes de la technologie blockchain. Le projet fournit des outils et services que les entreprises, les associations, et même les gouvernements peuvent utiliser facilement et intuitivement pour mettre en œuvre la technologie blockchain, depuis la création et la gestion d\'Actifs Intelligents à la réalisation et la distribution numérique.';
$i8n['main_about_title1']='Contrats Intelligents & DApps';
$i8n['main_about_desc1']='Des applications diversifiées, dynamiques et décentralisées, opérant sur le blockchain Expanse, allant des marchés décentralisés, registres globaux, accords imposés par calcul, jusqu\'à toute structure opérant exclusivement sur la blockchain.';
$i8n['main_about_title2']='Le TOUT décentralisé.';
$i8n['main_about_desc2']='Stockage Décentralisé de Données, Tenue de Registres, Traitement de l\'Information, Actifs Intelligents, et plus encore. Expanse permet un monde innovant bâti sur sa technologie distribuée.';
$i8n['main_about_title3']='Rapide. Intuitif. Puissant.';
$i8n['main_about_desc3']='La technologie blockchain rencontrent les Contrats Intelligents Complexes pour vous apporter des résultats sans précédents. Vitesse exponentiellement améliorée, fiabilité et performance permettent des réductions de coût drastiques en comparaison des solutions traditionnelles. ';
$i8n['main_about_title4']='Communauté Expanse';
$i8n['main_about_desc4']='Le Projet Expanse est géré par une structure décentralisée opérant sur le Blockchain Expanse. Cette entité est responsable des décisions significatives comme les nouveautés ou mises-à-jour sur lesquelles doivent se concentrer les développeurs.';

//Main Page : Downloads
$i8n['main_downloads_header']='Téléchargements';
$i8n['main_downloads_gexptitle']='gexp - core reference implementation';
$i8n['main_downloads_gexpdesc']='Implémentation primaire Expanse. Développée sous GO, c\'est le moyen idéal pour exploiter un nœud Expanse ou comme backend d\'Applications Expanse Distribuées.';
$i8n['main_downloads_wallettitle']='expwallet rich graphic wallet client';
$i8n['main_downloads_walletdesc']='Client d\'affichage intuitif et pratique des comptes et transactions, permettant l\'accès direct aux fonctionnalités Expanse les plus utilisées.';
$i8n['main_downloads_soon']='Bientôt Disponible';
$i8n['main_downloads_download']='Télécharger';
$i8n['main_downloads_close']='Fermer';

//Main Page : Explore
$i8n['main_explore_header']='<strong>Explorer</strong> un nouveau monde';

//Main Page : Documentation
$i8n['main_documentation_header']='Documentation';
$i8n['main_documentation_title']='Ce que vous devez savoir';
$i8n['main_documentation_desc']='En apprendre plus sur le projet Expanse et sa technologie. Du guide de démarrage pour les nouveaux utilisateurs aux spécifications techniques détaillées, un panel de documents est disponible pour vous aider à utiliser et comprendre Expanse';
$i8n['main_documentation_start']='Premiers pas';
$i8n['main_documentation_install']='Installation';
$i8n['main_documentation_concepts']='Concepts de base';
$i8n['main_documentation_backup']='Sauvegarde &amp; Restauration';
$i8n['main_documentation_mining']='Lancement rapide du Minage';
$i8n['main_documentation_accounts']='Gestion des Comptes';
$i8n['main_documentation_tx']='Envoyer des Transactions';
$i8n['main_documentation_tech']='Documentations Techniques';
$i8n['main_documentation_guide']='Guide utilisateur';
$i8n['main_documentation_roadmap']='RoadmapFR';
$i8n['main_documentation_whitepaper']='Whitepaper';

//Main Page : Team
$i8n['main_team_heading']='Rencontrez l\'Équipe';
$i8n['main_team_title_chris']='Lead Développeur ';
$i8n['main_team_desc_chris']='Chris est un développeur de cryptodevises avec un haut niveau d\'expertise de la technologie blockchain. Il a une connaissance variée de l\'industrie et une forte réputation dans la communauté.';
$i8n['main_team_title_dan']='Co-Développeur';
$i8n['main_team_desc_dan']='Dan est un fervent partisan et développeur de la technologie blockchain, fournissant du conseil pour un différents projets et sociétés dans l\'espace technologique des cryptodevises et de la blockchain.';
$i8n['main_team_title_james']='Manageur Communauté';
$i8n['main_team_desc_james']='James est un analyste, investisseur, et enthousiaste des cryptodevises, et est fondateur d\'une des plus grandes et actives communautés de discussions crypto du monde. Community manager expérimenté, James offre ses services à de nombreux projets sérieux de technologie blockchain.';
$i8n['main_team_title_marcia']='Stratégie marketing';
$i8n['main_team_desc_marcia']='Marcia Danzeisen a dirigé le marketing et la stratégie pour certaines des plus grosses banques et sociétés technologiques de services financiers du monde. Ecrivaine accomplie, Danzeisen a remarqué comment les cryptodevises et la technologie blockchain ont changé le monde traditionnel de la FinTech, et elle s\'est alors immergée dans cette voie.';
$i8n['main_team_title_esias']='Développement commerce int. - Chine';
$i8n['main_team_desc_esias']=' Esaias est expérimenté dans la liaison d\'affaires et spécialisé dans les marchés mondiaux. En tant qu\'ancien VP Opérations pour Cryptoexchange Inc., il a une forte expérience de l\'industrie qui vient compléter ses connaissances en développement de logiciels et d\'affaires.';
$i8n['main_team_title_tim']='Développement commerce int. - France';
$i8n['main_team_desc_tim']='Timothée est un crypto-enthousiaste autodidacte. Exploitant l\'analyse en ondelettes combinée à la TA traditionnelle pour le traitement des signaux financiers, il rédige des articles d\'analyses de marché pour un webzine et  participe à des meetups crypto à Bordeaux, France.';
$i8n['main_team_title_sandro']='Directeur artistique';
$i8n['main_team_desc_sandro']='Sandro a travaillé avec plusieurs sociétés de blockchain basées partout dans le monde. Il est talentueux et qualifié concernant le design et la conceptualisation de visuels. Un expert dans la création d\'interfaces et de sites web, l\'animation graphique et 3D, et les arts créatifs.';
$i8n['main_team_title_dave']='Gourou officiel du forum';
$i8n['main_team_desc_dave']='Dave est un enthousiaste des communautés et des blockchains avec de nombreuses années d\'expérience dans la modération de forums et de communautés.';

//Main Page : Community 
$i8n['main_community_heading']='Communauté';
$i8n['main_community_title']='Forum de la Communauté';
$i8n['main_community_desc']='Le <a href="//forum.expanse.tech"><span class="link_white">Forum</span></a> Expanse est le meilleur endroit pour discuter de Expanse dans les détails et connaitre les conversations historiques concernant le projet.';
$i8n['main_community_featured']='Les Sous-Forums majeurs';
$i8n['main_community_subforum1']='Nouvelles &amp; Annonces';
$i8n['main_community_subforum2']='Développement &amp; Technique';
$i8n['main_community_subforum3']='Contrats &amp; Applications Décentralisées';
$i8n['main_community_interact']='Interagir en direct';
$i8n['main_community_interact_desc']='La communauté est très active sur Slack. Si vous désirez participer ou discuter de Expanse avec la communauté et les développeurs.';
$i8n['main_community_social']='Media &amp; Blog';
$i8n['main_community_blog']='BLOG EXPANSE';
